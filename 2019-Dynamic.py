...
def V2(t, b, m):
    ...
    if m == 1:
        return max(
            (
                12*min(2*E[t], b+10*a) - 50*a - 0.5*max(0, b + 10*a - 2*E[t]) + V2(t+1, max(0, b + 10*a - 2*E[t]), 1)
            )
            for a in range(4)
        )
    ...
    return max(
        (
            0.7 * (12*min(E[t], b+10*a) - 50*a - 0.5*max(0, b + 10*a - E[t]) + V2(t+1, max(0, b + 10*a - E[t]), 0)) +
            0.3 * (12*min(2*E[t], b+10*a) - 50*a - 0.5*max(0, b + 10*a - 2*E[t]) + V2(t+1, max(0, b + 10*a - 2*E[t]), 1))
        )
        for a in range(4)
    )
...
print("\n\n\033[1m Solution Prepared for 2019 MATH3202 Practical Exam Question 2(b). Minimum combined assignment and build cost is $" + str(sol) + "\033[0m.\n")

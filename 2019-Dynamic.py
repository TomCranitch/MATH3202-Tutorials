...
def V1(t, b):
    ...
    return max(
        (
            12*min(E[t], b+10*a) - 50*a - 0.5*b + V1(t+1, max(0, b + 10*a - E[t]))[0],
            (a,) + V1(t+1, max(0, b + 10*a - E[t]))[1]
        )
        for a in range(4)  # Never want to buy excess books due to storage between weeks (we want at most 30 books at a time)
    )
...
#   and another tuple as the second element which contains the optimal buying pattern if no movie is ever announced or
...
def V2(t, b, m):
    ...
    if m == 1:
        return max(
            (
                12*min(2*E[t], b+10*a) - 50*a - 0.5*b + V2(t+1, max(0, b + 10*a - 2*E[t]), 1)[0],
    ...
    return max(
        (
            0.7 * (12*min(E[t], b+10*a) - 50*a - 0.5*b + V2(t+1, max(0, b + 10*a - E[t]), 0)[0]) +
            0.3 * (12*min(2*E[t], b+10*a) - 50*a - 0.5*b + V2(t+1, max(0, b + 10*a - 2*E[t]), 1)[0]),
